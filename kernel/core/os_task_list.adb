...
is
   ...
   function os_mbx_get_waiting_mask
     ...
     (os_task_list_rw (task_id).mbx_waiting_mask);
   ...
   procedure os_sched_add_task_to_ready_list (task_id : os_task_id_param_t) is
   ...
   --  the first task of the list should not have any prev task.
       and then
         os_task_list_rw (os_sched_get_current_list_head).prev =
         OS_TASK_ID_NONE
   ...
   --  The next needs to have the actual task as prev
             and then
               os_task_list_rw (os_task_list_rw (task_id).next).prev =
               task_id
   --  prev and next need to be different
             and then
               os_task_list_rw (task_id).next /=
