...
package os_arch with
 ...
 is
   ...
   function os_arch_interrupt_is_pending return types.uint8_t with
      Global => null;
   ...
   procedure os_arch_context_create (task_id : os_task_id_param_t) with
      Global => null;
   pragma Import (C, os_arch_context_create, "os_arch_context_create");

   procedure os_arch_context_switch (prev_id : os_task_id_param_t; next_id : os_task_id_param_t) with
      Global => null;
   pragma Import (C, os_arch_context_switch, "os_arch_context_switch");

   procedure os_arch_context_set (task_id : os_task_id_param_t) with
      Global => null;
   pragma Import (C, os_arch_context_set, "os_arch_context_set");

   procedure os_arch_space_init with
      Global => null;
   pragma Import (C, os_arch_space_init, "os_arch_space_init");

   procedure os_arch_space_switch (old_context_id : os_task_id_param_t; new_context_id : os_task_id_param_t) with
      Global => null;
   pragma Import (C, os_arch_space_switch, "os_arch_space_switch");

   procedure os_arch_cons_init with
      Global => null;
