...
package body os_task_mbx with
   ...
   function "+"
     (Left  : os_mbx_index_t;
      Right : os_mbx_count_t) return os_mbx_index_t is
   ...
   function os_mbx_is_empty
     (task_id : os_task_id_param_t) return Boolean is
     (os_task_mbx_rw (task_id).count = os_mbx_count_t'First);

   --------------------
   -- os_mbx_is_full --
   --------------------

   function os_mbx_is_full
     (task_id : os_task_id_param_t) return Boolean is
     (os_task_mbx_rw (task_id).count = os_mbx_count_t'Last);
   ...
   procedure os_mbx_inc_mbx_head (task_id : os_task_id_param_t) is
   begin
      os_task_mbx_rw (task_id).head :=
   ...
   procedure os_mbx_inc_mbx_count (task_id : os_task_id_param_t) is
   begin
      os_task_mbx_rw (task_id).count :=
   ...
   procedure os_mbx_dec_mbx_count (task_id : os_task_id_param_t) is
   begin
      os_task_mbx_rw (task_id).count :=
        os_mbx_count_t'Pred (os_mbx_get_mbx_count (task_id));
   ...
   function os_mbx_get_tail_sender
     (task_id : os_task_id_param_t) return os_task_id_t is
     (os_task_mbx_rw (task_id).mbx_array
        (os_mbx_get_mbx_head (task_id) + os_mbx_get_mbx_count (task_id))
        .sender_id);
   ...
   function os_mbx_get_mbx_entry
     (task_id   : os_task_id_param_t;
      mbx_index : os_mbx_index_t) return os_mbx_entry_t is
     (os_task_mbx_rw (task_id).mbx_array (mbx_index));
   ...
   procedure os_task_mbx_init (task_id : os_task_id_param_t) is
   begin
   ...
   function os_ghost_task_mbx_are_well_formed
     ...
     (for all index in os_mbx_index_t'Range =>
        (if
           (((not os_mbx_is_empty (task_id))
             and then
             (os_ghost_get_mbx_tail (task_id) < os_mbx_get_mbx_head (task_id))
             and then
         ...
         then
           os_task_mbx_rw (task_id).mbx_array (index).sender_id in
             os_task_id_param_t
         else
